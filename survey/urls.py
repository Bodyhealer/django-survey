...
urlpatterns = [
    ...
    url(r"^(?P<uuid>[0-9a-f-]+)/", SurveyDetail.as_view(), name="survey-detail"),
    url(r"^csv/(?P<primary_key>\d+)/", serve_result_csv, name="survey-result"),
    url(r"^(?P<uuid>[0-9a-f-]+)/completed/", SurveyCompleted.as_view(), name="survey-completed"),
    url(r"^(?P<uuid>[0-9a-f-]+)-(?P<step>\d+)/", SurveyDetail.as_view(), name="survey-detail-step"),
